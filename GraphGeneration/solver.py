  ...
  def update_fn(self, x, adj, flags, t):
    ...
    if self.obj == 'x':
      for i in range(n_steps):
        grad = score_fn(x, adj, flags, t)
        noise = gen_noise(x, flags, sym=False)
        momentum_buffer_x_norm = torch.norm(momentum_buffer_x.reshape(grad.shape[0], -1), dim=-1).mean()
        noise_norm = torch.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
        ...
        if momentum_buffer_x_norm == 0:
          ...
          grad_diff = grad - grad_pre_x
          grad_diff_norm = torch.norm(grad_diff.reshape(grad_diff.shape[0], -1), dim=-1).mean()
          beta_1 = max(0, min(1.-eps, ((1.-step_adam_x[:, None, None, None].mean()*(grad_diff_norm / momentum_buffer_x_norm))
                                      /(1.+step_adam_x[:, None, None, None].mean()*(grad_diff_norm / momentum_buffer_x_norm)))**2))
          m_term_x = beta_1 * m_term_x + (1 - beta_1) * grad
          m_term_x_norm = torch.norm(m_term_x.reshape(m_term_x.shape[0], -1), dim=-1).mean()
          step_size = (target_snr * noise_norm / m_term_x_norm) ** 2 * 2 * alpha
      ...
      for i in range(n_steps):
        grad = score_fn(x, adj, flags, t)
        noise = gen_noise(adj, flags)
        momentum_buffer_adj_norm = torch.norm(momentum_buffer_adj.reshape(grad.shape[0], -1), dim=-1).mean()
        noise_norm = torch.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
        ...
        if momentum_buffer_adj_norm == 0:
          ...
          grad_diff = grad - grad_pre_adj
          grad_diff_norm = torch.norm(grad_diff.reshape(grad_diff.shape[0], -1), dim=-1).mean()
          
          beta_1 = max(0, min(1.-eps, ((1.-step_adam_adj[:, None, None, None].mean()*(grad_diff_norm / momentum_buffer_adj_norm))
                                      /(1.+step_adam_adj[:, None, None, None].mean()*(grad_diff_norm / momentum_buffer_adj_norm)))**2))
          m_term_adj = beta_1 * m_term_adj + (1 - beta_1) * grad
          m_term_adj_norm = torch.norm(m_term_adj.reshape(m_term_adj.shape[0], -1), dim=-1).mean()
          step_size = (target_snr * noise_norm / m_term_adj_norm) ** 2 * 2 * alpha
...
# -------- PC sampler --------
def get_pc_sampler(sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='Langevin', 
  ...
  def pc_sampler(model_x, model_adj, init_flags):
    ...
    elif corrector == 'Crimomentum':
      corrector_fn = Crimomentum
    else:
